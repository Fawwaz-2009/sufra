  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        data-size={size}
        // `max-h-[85svh]` + internal scroll mirrors the Sheet fix: a
        // confirmation dialog with a long description (or one shown on
        // a small phone with the keyboard up) used to extend past the
        // screen edge with the buttons hidden. Bound it to 85% of the
        // small-viewport-height and scroll inside if needed.
        className={cn(
          "group/alert-dialog-content fixed top-1/2 start-1/2 z-50 grid max-h-[85svh] w-full -translate-x-1/2 rtl:translate-x-1/2 -translate-y-1/2 gap-6 overflow-y-auto overscroll-contain rounded-xl bg-popover p-6 text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-lg data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className,
        )}
        {...props}
      />
