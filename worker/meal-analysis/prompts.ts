// Locale design (multi-language per PRD §6.10):
//
// We keep ONE English system prompt as the source of truth. When the user's
// locale isn't English, we append a short instruction telling the model to
// output specific user-read fields in the target language. Modern vision
// models (Gemini 3, GPT-5.4, Claude 4.5+) follow per-field language
// instructions reliably; verified by the locale-check eval row.
//
// Adding a new language = add its name to LOCALE_NAMES below. No prompt
// translation needed, no new prompt file. Two-line change per language.
...
// User-read fields — the ONLY fields that get localized. Everything else
// (units, numeric values, IDs) stays English / standard.
...
// User-message builder. Always English (the model is told to localize the
// output fields, not its own framing). Single-pass only — no clarification
// round-trip until M4 designs the actual UX.
export function buildUserPromptText(parts: UserPromptParts): string {
