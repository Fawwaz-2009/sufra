...
const HEIGHT_UNIT_VALUES = ["cm", "imperial"] as const
const WEIGHT_UNIT_VALUES = ["kg", "lb"] as const

const onboardingBodySchema = z.object({
  sex: z.enum(SEX_VALUES),
  birthday: z.iso.date(),
  heightCm: z.number().int().min(100).max(250),
  displayHeightUnit: z.enum(HEIGHT_UNIT_VALUES).default("cm"),
  weightKg: z.number().min(30).max(300),
  displayWeightUnit: z.enum(WEIGHT_UNIT_VALUES).default("kg"),
...
const profilePatchBodySchema = z.object({
  ...
  displayHeightUnit: z.enum(HEIGHT_UNIT_VALUES).optional(),
  weightKg: z.number().min(30).max(300).optional(),
  displayWeightUnit: z.enum(WEIGHT_UNIT_VALUES).optional(),
...
const authedMember: MiddlewareHandler<{
  Bindings: AppEnv
...
const app = new Hono<{
  ...
  // Member-auth gate for profile + onboarding endpoints. Same shape as the
  // /api/meals/* middleware (read session, 401 if missing, attach to c.var).
  .use("/api/profile", authedMember)
  .use("/api/onboarding", authedMember)
  ...
  .post(
    "/api/onboarding",
    zValidator("json", onboardingBodySchema, onInvalidInput),
    async (c) => {
      const body = c.req.valid("json")
  ...
  .patch(
    "/api/profile",
    zValidator("json", profilePatchBodySchema, onInvalidInput),
    async (c) => {
      const body = c.req.valid("json")
      const userId = c.var.session.user.id
      const db = createDb(c.env.DB)
